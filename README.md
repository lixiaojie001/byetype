...
文本优化是转写后的二次处理，用于优化格式和排版（如自动换行、翻译等）。在通用设置中为快捷键绑定输出风格即可启用。
...
| 快捷键 | 功能 | 默认绑定模板 | 修改方式 |
|--------|------|-------------|----------|
| **F4** | 语音输入 1（按一次开始录音，再按一次停止并转写） | 自动换行 | 设置 → 通用设置 |
| 未设置 | 语音输入 2 | 翻译 | 设置 → 通用设置 |
| **F6** | 截图取词（截图选区 → AI 识别 → 复制到剪贴板） | 文字识别 | 设置 → 通用设置 |
| 未设置 | 截图翻译 | 翻译 | 设置 → 通用设置 |

每个快捷键可绑定不同的输出风格（自动换行、翻译、自定义等），在通用设置中配置。

### 📸 图像文字提取

按 **F6**（截图取词）触发截图选区，框选屏幕上的任意区域，AI 自动识别文字并复制到剪贴板。识别结果会在预览窗口中展示，支持固定（Pin）预览窗口。效果对比见上方 [图像文字提取 — 不只是 OCR](#-图像文字提取--不只是-ocr) 章节。
