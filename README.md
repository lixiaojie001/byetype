...
| 快捷键 | 功能 | 修改方式 |
|--------|------|----------|
| **F4** | 语音录制（按一次开始，再按一次停止并转写） | 设置 → 通用设置 → 录音快捷键 |
| **F6** | 图像文字提取（截图选区 → AI 识别 → 复制到剪贴板） | 设置 → 通用设置 → 提取快捷键 |

### 📸 图像文字提取

按 **F6** 触发截图选区，框选屏幕上的任意区域，AI 自动识别文字并复制到剪贴板。识别结果会在预览窗口中展示，支持固定（Pin）预览窗口。效果对比见上方 [图像文字提取 — 不只是 OCR](#-图像文字提取--不只是-ocr) 章节。
...
**配置方式：**
- 提取模型：设置 → 图像识别 → 模型选择（可覆盖默认转写模型）
- 提取提示词：设置 → 图像识别 → 提示词编辑
- 思考模式：设置 → 图像识别 → 思考模式（独立于语音转写的思考模式设置）
...
**图像文字提取：**
```
📸 截图 → 🔍 文字识别（text-extract.md）→ 📋 复制到剪贴板
```
...
| 🔍 图像提取（text-extract.md） | 控制截图文字识别 | 定义 OCR 行为，包括布局理解、格式保持等规则 |
