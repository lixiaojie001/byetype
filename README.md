...
AI 驱动的语音输入工具，告别打字，用说的 🎙️

ByeType 是一个轻量级桌面应用，目标是**用语音输入完全取代键盘打字** — 无论是写文档、回消息、写代码注释还是填表单，所有需要打字的地方都可以直接说出来。按下快捷键 🎤 开始说话，AI 将语音实时转录为文本并自动粘贴到光标位置。它的核心特色是**提示词驱动的高可定制性** — 通过编辑 Markdown 格式的提示词文件，你可以自定义转录的各种行为：

- 自动换行 — 口述一整段话，AI 自动分段排版，每句话自动换行
- 自动翻译 — 说"我觉得这个方案的可行性比较高，建议优先推进"，输出 "I think this plan is highly feasible and recommend prioritizing it"
- 数字与单位格式化 — 说"第一步花了九块五毛买了三千五百米的线材"，输出"第1步花了9.5元买了3500米的线材"
- 计算式转换 — 说"四除以三等于零点七五"，输出"4/3=0.75"
- 专有词汇校正 — 说"我们用deep seek的模型跑在苹果的mac book pro上用cursor写代码"，输出"我们用DeepSeek的模型跑在Apple的MacBook Pro上用Cursor写代码"
- 口语清理 — 说"嗯……就是说那个我觉得吧，基本上这个方案其实还是可以的，如果需要的话可以联系我"，输出"我觉得这个方案还是可以的，如果需要可以联系我"
- 符号口令 — 说"零八下划线report圆圈1完成圆圈2待定双横杠结束"，输出"08_report①完成②待定--结束"
- 字母大写 — 说"请选择选项a b c d"，输出"请选择选项ABCD"

适合写作、笔记、编程注释、即时通讯等一切需要大量文字输入的场景 ✍️
...
3. 点击左侧栏「模型管理」
4. 在预置模型卡片中填写你的 API Key（获取方式见下方「AI 模型配置」）
5. 点击左侧栏「语音转写」，选择转写模型（默认 Gemini 3.0 Flash）
6. 关闭设置窗口
...
ByeType 内置以下模型，使用前需要获取对应的 API Key：

| 模型 | API ID | 需要的 Key | 获取方式 |
|------|--------|-----------|----------|
| Gemini 3.0 Flash | `gemini-3-flash-preview` | Gemini API Key | 前往 [Google AI Studio](https://aistudio.google.com/) 注册并创建 API Key |
| Gemini 3.1 Flash Lite | `gemini-3.1-flash-lite-preview` | Gemini API Key | 同上 |
| Qwen3 Omni Flash | `qwen3-omni-flash` | Qwen API Key | 前往[阿里云百炼](https://bailian.console.aliyun.com/)注册并创建 API Key |

API Key 填写位置：设置 → 模型管理 → 对应模型卡片

**如何选择模型：**
- **Gemini 3.0 Flash**：推荐，速度和质量均衡
- **Gemini 3.1 Flash Lite**：更快速，适合对延迟敏感的场景
...
> 注意：思考模式仅支持 Gemini 协议的模型，其他协议模型该选项不显示。
